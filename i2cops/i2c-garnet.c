...
static int write_byte(int fd, unsigned char saddr, uint8_t * buf, int size)
{
	struct i2c_msg msgs[] = {
		[0] = {
			.addr = saddr,
...
int main(int argc, char * argv[]) 
{
	...
	while ((c = getopt(argc, argv, "b:dhlt:wn:v")) != -1) {
		switch (c) {
			case 'b':
				bus_num = atoi(optarg);
				break;
	...
	if(read == 0)
	{
		printf("try to write: slave addr - 0x%x,data len is : %d,data:",saddr,n_data);
		for(i = 0;i < n_data;i++)
		{
			data_buf[i] =  str2int(argv[optind + i + 1]);
			printf(" 0x%x",data_buf[i]);
		}
		printf(" \n");
	}
	else
	{
		printf("try to read: slave addr - 0x%x,data len is : %d\n",saddr,n_data);
	}
	do {
		if (read)
 		{
			if (0 == read_byte(fd, saddr, &data_buf[0], n_data))
			{
				printf("read success!\n");
			}
		...
		{
			if (0 == write_byte(fd, saddr,&data_buf[0], n_data)) {
